...
#define PLUGIN_URI "http://moddevices.com/plugins/mod-devel/simple-cv-button"

#define N_PROPS             2
#define MAX_STRING          1024

#define PRESS_STRING_URI    PLUGIN_URI "#port1string"
#define CV_STATE_URI        PLUGIN_URI "#CVStateMask"

#define SPECIAL_PORT_RESET  UINT8_MAX

#define PRESS_TXT           "PRESS"

//timing configs
#define CHANGE_COUNTER      1400

//status bitmasks
#define PRESS_ON            0x01
...
map_uris(LV2_URID_Map* map, URIs* uris)
{
    ...
    uris->port1_string       = map->map(map->handle, PRESS_STRING_URI);
    ...
    switch (port_index) {
        ...
        case CV_PRESS:
        {
            ...
            if (self->hmi->size >= LV2_HMI_WIDGETCONTROL_SIZE_POPUP_MESSAGE) {
                if (self->cv_value)
                    self->hmi->popup_message(self->hmi->handle, self->toggle_addressing, LV2_HMI_Popup_Style_Inverted, PRESS_TXT, label);
                else
                    self->hmi->popup_message(self->hmi->handle, self->toggle_addressing, LV2_HMI_Popup_Style_Normal, PRESS_TXT, label);
            }
            else {
            ...
            const LV2_Feature* const* features)
{
    ...
    state_map_init(
        ...
        PRESS_STRING_URI, STATE_MAP_INIT(String, &state->port1string),
        ...
        const LV2_Feature* const*   features)
{
  ...
  if (self->state.CV_mask_value & PRESS_ON)
      self->cv_value = 10.f;
             ...
             void*      data)
{
    ...
    switch ((PortIndex)port) {
        case BUTTON:
            ...
            break;
        case CV_PRESS:
            self->cv_press = (float*)data;
            break;
        case PRESS_MODE:
            self->press_mode = (const float*)data;
            break;
        case PARAMS_IN:
...
run(LV2_Handle instance, uint32_t n_samples)
{
    ...
    uint8_t press_mode = (int)*self->press_mode;
    ...
    if (self->prev_button_value != button_value) {
        ...
        if (button_value > 0) {
            switch (press_mode) {
                case LATCHING:
                    self->cv_value = 10.f - self->cv_value;
                break;
            ...
            trigger_widget_change(self, CV_PRESS);
        }
        //button released
        else {
            //momentary modes revert on release
            if (press_mode == MOMENTARY_ON) {
                self->cv_value = 0.f;
                trigger_widget_change(self, CV_PRESS);
            }
            else if (press_mode == MOMENTARY_OFF) {
                self->cv_value = 10.f;
                trigger_widget_change(self, CV_PRESS);
            }
        }
    ...
    for(uint32_t i = 0; i < n_samples; i++) {
        ...
        self->cv_press[i] = self->cv_value;
    }

    self->button_mask = 0;
    if (self->cv_value)
        self->button_mask |= PRESS_ON;
